...
#: Контракт уезжает хосту в рукопожатии: это первое, что он про нас узнаёт.
INSTRUCTIONS = """AgentOS — система автономных агентов с состоянием на диске.
...
1. В начале каждой сессии вызови `agentos_resume`. Одна строка ответа
   скажет, есть ли незавершённая работа, чего она ждёт и когда продолжит.
   Если работа есть — продолжай её, не спрашивая человека «что дальше».
2. Новая задача от человека, сколь угодно абстрактная, — `agentos_goal`.
   Система сама зафиксирует критерии приёмки, построит план и выдаст
   задания субагентам.
3. Каждое выданное задание выполняй своим механизмом субагентов, а
   результат возвращай через `agentos_task_report` — отчётом, а не
   транскриптом. Уложись в лимит из брифа: на нём держится экономия
   контекста.
4. Упёрся в лимит токенов, не хватает доступа или нужно подтверждение —
   `agentos_task_block`. Заблокированная задача не останавливает остальные
   ветки плана, а по квоте продолжится сама после сброса лимитов.
5. Миссия закрывается только приёмкой: `agentos_verify`. Красный
   программный гейт отменяет любой вердикт, включая твой.

Память проекта доступна через `agentos_memory_search` и
`agentos_memory_write`. Не спрашивай человека о том, что можешь узнать сам
из памяти, репозитория или запуска команды."""
